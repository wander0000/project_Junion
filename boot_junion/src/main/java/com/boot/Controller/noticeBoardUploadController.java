...
@Controller
@Slf4j
public class noticeBoardUploadController {
	@Autowired
	private noticeBoardUploadService service;
	
	@PostMapping("/noticeUploadAjaxAction") //uploadAjaxAction
...
//	ResponseEntity : 파일 정보를 넘기기위해서 사용
	public ResponseEntity<List<noticeBoardAttachDTO>> uploadAjaxPost(MultipartFile[] uploadFile) {
		log.info("noticeUpload ajax post...");
		
		List<noticeBoardAttachDTO> list = new ArrayList<>();
...
//	@ResponseBody
	@GetMapping(value = "/noticeGetFileList")
...
//	모든 파일은 내부적으로 bit 값을 가짐(문서, 영상, 이미지, 소리)
	@GetMapping(value = "/noticeDownload")
