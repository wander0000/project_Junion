...
@Controller
@Slf4j
public class comBoardUploadController {
	@Autowired
	private comBoardUploadService service;
	
	@PostMapping("/compUploadAjaxAction") //uploadAjaxAction
//	public void uploadAjaxPost(MultipartFile[] uploadFile) {
//	ResponseEntity : 파일 정보를 넘기기위해서 사용
	public ResponseEntity<List<comBoardAttachDTO>> uploadAjaxPost(MultipartFile[] uploadFile) {
		log.info("comUpload ajax post...");
		
		List<comBoardAttachDTO> list = new ArrayList<>();
...
//			getSize : 업로드 되는 파일 크기
			...
			comBoardAttachDTO boardAttachDTO = new comBoardAttachDTO();
...
//		파일정보들을 list 객체에 담고, http 상태값은 정상으로 리턴
		return new ResponseEntity<List<comBoardAttachDTO>>(list, HttpStatus.OK);
	}
...
//	@ResponseBody
	@GetMapping(value = "/compGetFileList")
	public ResponseEntity<List<comBoardAttachDTO>> getFileList(@RequestParam HashMap<String, String> param) {
		log.info("@# getFileList()");
		log.info("@# param=>"+param);
		log.info("@# param=>"+param.get("board_no"));
		
		return new ResponseEntity<>(service.boardGetFileList(Integer.parseInt(param.get("board_no"))), HttpStatus.OK);
	}
	
//	모든 파일은 내부적으로 bit 값을 가짐(문서, 영상, 이미지, 소리)
	@GetMapping(value = "/compDownload")
